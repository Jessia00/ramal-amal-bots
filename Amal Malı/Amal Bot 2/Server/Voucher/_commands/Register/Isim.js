...
module.exports = {
    Isim: "isim",
    Komut: ["i","nick"],
    Kullanim: "isim <@user/ID> <İsim> <Yaş>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.channel.send(cevaplar.üye + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@user/ID> <Isim> <Yaş>\``);
    ...
    if(!isim || !yaş) return message.channel.send(cevaplar.argümandoldur + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@user/ID> <Isim> <Yaş>\``);
...
${isimler}\n\nÜyesinin önceki isimlerine \`${sistem.botSettings.Prefixs[0]}isimler <@user/ID>\` komutuyla bakarak kayıt işlemini\n gerçekleştirmeniz önerilir.`
: "." : "."}`)]}).then(msg => {
