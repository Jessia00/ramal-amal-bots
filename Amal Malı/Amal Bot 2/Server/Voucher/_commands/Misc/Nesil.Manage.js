...
module.exports = {
    Isim: "nesil",
    Komut: ["nesil","nesilver"], 
    Kullanim: "rol <@user/ID>",
    Aciklama: "",
    Kategori: "yönetim",
    Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!roller.nesilRolleri) return message.channel.send(cevaplar.notSetup);
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0])
    if(!uye) return message.react(message.guild.emojiGöster(emojiler.Iptal));
    let rolList = [];
    let oyunRolleri = message.guild.roles.cache.sort((a, b) => b.position - a.position).filter(rol => roller.nesilRolleri.some(x => rol.id == x) && message.member.roles.highest.comparePositionTo(message.guild.roles.cache.get(rol.id)) > 1).forEach(x => {
      rolList.push([
