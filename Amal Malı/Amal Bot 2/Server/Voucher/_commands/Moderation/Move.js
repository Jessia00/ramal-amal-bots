...
module.exports = {
    Isim: "taşı",
    Komut: ["kanal-taşı"],
    Kullanim: "taşı <@user/ID> <Kanal ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.channel.send(cevaplar.üye + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@user/ID> <Kanal ID>\``);
    let kanal = message.guild.channels.cache.get(args[1]);
    if(!kanal) return message.channel.send(cevaplar.argümandoldur + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@user/ID> <Kanal ID>\``);
