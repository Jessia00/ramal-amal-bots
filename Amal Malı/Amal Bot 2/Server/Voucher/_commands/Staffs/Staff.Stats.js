...
module.exports = {
  ...
  onLoad: function (client) {
    ...
    client.seyfoSaatYap = (date) => { return moment.duration(date).format('H'); };
  },
    ...
    if(görevBul) {
      ...
      if(data) {
        ...
        data.taskVoiceStats.forEach((value, key) => {
            if(key == kanallar.publicKategorisi) public += Number(client.seyfoSaatYap(value))
            if(key == kanallar.streamerKategorisi) public += Number(client.seyfoSaatYap(value))
        });
        data.taskVoiceStats.forEach((value, key) => {
          if(key == kanallar.registerKategorisi) register += Number(client.seyfoSaatYap(value))
        })
      };
...
if(görevBul.publicVoice >= 1 && görevBul.AllVoice >= 1) {
        ...
        yönetimPaneli.addFields({ name: `${görevBul.AllVoice} Saat Tüm Ses Kanallarında Takıl!`, value: `${Number(client.seyfoSaatYap(genelses)) >= görevBul.AllVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Mesaj) } ${yönetimBar(Number(client.seyfoSaatYap(genelses)) ? Number(client.seyfoSaatYap(genelses)) : 0, görevBul.AllVoice, 5, Number(client.seyfoSaatYap(genelses)))} \`${Number(client.seyfoSaatYap(genelses)) >= görevBul.AllVoice ? `Tamamlandı!`: `${Number(client.seyfoSaatYap(genelses))} saat / ${görevBul.AllVoice} saat`}\``})
       }
