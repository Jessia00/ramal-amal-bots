...
async function hoşgeldinMesajı(member) {
    ...
    if (invite === null) {
      ...
      if(hoşgeldinKanal) hoşgeldinKanal.send(`**${_set.serverName ? `${_set.serverName}` : member.guild.name}'ya Hoş geldin ${member} biz de seni bekliyorduk, hesabın** __${global.tarihsel(member.user.createdAt)}__ **tarihinde** ${global.tarihHesapla(member.user.createdAt)} **oluşturulmuş!**
...
${member.guild.emojiGöster(emojiler.sarıYıldız)} **Sunucumuza kayıt olmak için <#998453516660117535> kanalına göz atmayı unutma.**

${_set.taglıalım ? `
${member.guild.emojiGöster(emojiler.sarıYıldız)} **Sunucumuzdaki gelişmelerden haberdan olmak** <#1015699224815489114> **kanalına bakmayı unutma.**
...
async function rolTanımlama(üye, rol) {
    ...
    await üye.setRoles(rol).then(async (seyfo) => {
