...
${limitController.sort((a, b) => b.date - a.date).map((x, index) => `${index+1}. | ${x.proc} | ${tarihsel(x.date)}`).join("\n")}
        ...
        async queryManage(oldData, newData) {
            ...
            if(sunucuData) {              
                ...
                if(oldData === sunucuData.katarsis) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.katarsis": newData}})
                }
                ...
                if(oldData === sunucuData.kufurİzinliKanal) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.kufurİzinliKanal": newData}})
                }
                ...
                if(sunucuData.rolPanelRolleri.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.sorumluRolleri": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.sorumluRolleri": newData}})
                }
                if(sunucuData.rolPanelRolleri.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.nesilRolleri": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.nesilRolleri": newData}})
                }
        ...
        connect(token) {
            ...
            this.login(token).then(seyfo => {
                ...
                this.user.setActivity('Değer Mi' ,  { type: 'STREAMING', url: 'https://www.twitch.tv/seyfooksck'})
                this.on("ready", async () => { 
                    let Whitelist = await GUARD_SETTINGS.findOne({guildID: sistem.SERVER.ID})
                    if(!Whitelist) {
