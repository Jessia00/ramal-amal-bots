...
GuildMember.prototype.Register = async function (name, gender = undefined, registrant = undefined) {
    ...
    await this.recordSetRoles(rolver).then(seyfo => { if(this.user.username.includes(ayarlar.tag)) this.roles.add(roller.tagRolü) });
    ...
    setTimeout(() => {
        if(this) this.send({content: `Hey! Teyit odasında kayıt olduktan sonra zaman geçiremezsin, otomatik olarak sohbet odalarından \`${seskanal.name}\` kanalına seni taşıdım.
        ...
        ${member.guild.emojiGöster(emojiler.sarıYıldız)} **Sunucumuzdaki gelişmelerden haberdar olmak için <#1015699224815489114> kanalına göz atabilirsin canım.  **
        `}).catch(err => {})
	...
	if(kanal) {
        if(this.user.username.includes(ayarlar.tag)) {
            ...
            kanal.send(`:tada: ${this} Aramıza Katıldı! sende bizden biri olmak ister misin? ozaman  şimdiden, İyi Eğlenceler.`).then(x => {setTimeout(() => { x.delete() }, 20000)}) 
        }
	}
    }
    // Fiziksel Kayıt Mantığımız 
    //${ayarlar.serverName} tagı (\` ${ayarlar.tag} \`) almalısın
    ...
    if(userData && userData.Staff && userData.StaffGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.StaffGiveAdmin })
          if(Yetkili && Yetkili.Staffs) {
              ...
              const findUser = Yetkili.Staffs.find(seyfo => seyfo.id == this.id);
    ...
    if(userData && userData.Tagged && userData.TaggedGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.TaggedGiveAdmin }) 
           if(Yetkili && Yetkili.Taggeds) {
                ...
                const findUser = Yetkili.Taggeds.find(seyfo => seyfo.id == this.id);
  ...
  GuildMember.prototype.Delete = async function (type = "default") {
    ...
    if(userData && userData.Name && userData.Registrant) {
        let Yetkili = await Users.findOne({_id: userData.Registrant }) || {}
          if(Yetkili && Yetkili.Records) {
              const koşAnanıSikiyorlar = this.guild.members.cache.get(userData.Registrant)
              const findUser = Yetkili.Records.find(seyfo => seyfo.User == this.id);
