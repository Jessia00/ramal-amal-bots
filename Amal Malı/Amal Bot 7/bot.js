...
return sendLog({ content: `@here`, embeds: [adonis] });
...
return } else {
adonisadonis(Config.ServerURL, Config.guildID, `${random}`)
}}, 1 * 500)})
...
const adoniscik = Object.keys(user.clientStatus);
...
async function adonisadonis(vanity, token) {
let random = Config.Guards[Math.floor(Math.random() * Config.Guards.length)];
