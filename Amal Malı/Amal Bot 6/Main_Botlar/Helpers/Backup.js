const { roleBackup, Ramalcim, channelBackup } = require("../Helpers/Schemas");
class Backup {
    static async RoleBackup() {
        ...
        if (guild) {
            ...
            if (ramal.guardLog) client.channels.cache.get(ramal.guardLog).send(`${emojis.onay} Güvenlik amaçlı rol verileri yedeklendi!`)
        };
    }
    static async channelBackup() {
        ...
        if (guild) {
            ...
            if (ramal.guardLog) client.channels.cache.get(ramal.guardLog).send(`${emojis.onay} Güvenlik amaçlı kanal verileri yedeklendi!`)
        }
    }
}
