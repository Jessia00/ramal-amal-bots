...
module.exports = async (client) => {
  ...
  const { Ramalcim } = global.ramal = require("../Helpers/Schemas")
    ...
    Discord.GuildMember.prototype.updateTask = async function (guildID, type, data, channel = null) {
      ...
      taskData.forEach(async (x) => {
        ...
        if (x.completedCount >= x.count) {
          ...
          if (ramal.dailyMissionLog) client.channels.cache.get(ramal.dailyMissionLog).send(`${this.toString()} Tebrikler! ${type.charAt(0).toLocaleUpperCase() + type.slice(1)} görevini başarılı bir şekilde tamamladın!\n**_______________________________**\n\nGörev: ${x.message}\nKazanılan Coin : **${x.prizeCount}**`)
        }
        await x.save();
      });
    };
