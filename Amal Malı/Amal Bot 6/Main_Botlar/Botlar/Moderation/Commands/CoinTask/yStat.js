const { Ramalcim, tasks, messageUser, voiceUser, coin, Users, Inviter, Penalties } = require('../../../../Helpers/Schemas')
...
class YStat extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!message.member.roles.cache.has(ramal.registerHammer) && !ramal.yonetimRoles.some(rol => message.member.roles.cache.has(rol))) return message.reply(`**UYARI: **Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`).sil(10);
        ...
        const coinStatus = ramal.coinSystem && message.member.roles.cache.has(ramal.registerHammer, false) && coinDatas.advancedRanks.length > 0 ?
            `
