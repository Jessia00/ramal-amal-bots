const { Ramalcim, tasks } = require("../../../../Helpers/Schemas")
...
class Tasks extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (args[0] == 'dağıt' || args[0] == 'dagit' || args[0] == 'dağit') {
            ...
            if (!role && !member.roles.cache.has(ramal.registerHammer)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** Bu üye bir yetkili değil!`)] }).sil(40);
            ...
            if (role) {
                const members = role.members.filter((x) => x.roles.cache.has(ramal.registerHammer));
                ...
                if (ramal.dailyMissionLog) client.channels.cache.get(ramal.dailyMissionLog).send(`${members.map((x) => x.toString()).join(", ")} üyelerine ${type} görevi dağıtıldı!`)
            } else {
                const id = await tasks.find({ guildID: message.guild.id });
                ...
                if (ramal.dailyMissionLog) client.channels.cache.get(ramal.dailyMissionLog).send(`${member.toString()} üyesine ${type} görevi verildi!`)
            }
