const { Ramalcim, Users, coin } = require('../../../../Helpers/Schemas')
class Tagges extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!message.member.roles.cache.has(ramal.registerHammer) && !config.Founders.includes(message.member)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** Bu komutu kullanmak için yeterli yetkin bulunmamakta!`)] }).sil(20)
        ...
        if (!ramal.tags.some(tag => member.user.username.includes(tag)) || member.user.discriminator == ramal.tags.filter(discrim => !isNaN(discrim))[0]) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** Belirttiğin kişi tagımızı taşımıyor!`)] }).sil(20);
        ...
        if (veri && veri.Taggeds.filter(a => a.userID === member.id)) {
            ...
            collector.on('collect', async (button, user) => {
                if (button.customId === "onayla") {
                    ...
                    client.channels.cache.get(ramal.tagLog).send({ embeds: [embed.setDescription(`${member} kişisi ${message.author} yetkilisi tarafından **${new Date(message.createdAt).toTurkishFormatDate()}** tarihinde taglı olarak belirlendi!`).setFooter("Developed By Ramal.")] })
                    if (ramal.coinSystem) await coin.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { coin: config.taggedCoin } }, { upsert: true });
