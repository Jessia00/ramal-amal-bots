...
class Herkese extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!role) return message.reply(`**UYARI :** Bir rol belirtmeyi unuttun! Rol belirtip tekrar dene!`).sil(10)
          ...
          if(ramal.denetimLog) await client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${role} isimli rol sunucuda bulunan tüm üyeler ${message.author} tarafından dağıtılmaya başlandı!`)]})
