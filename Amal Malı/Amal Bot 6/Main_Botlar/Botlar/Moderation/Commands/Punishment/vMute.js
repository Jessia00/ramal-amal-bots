...
class Vmute extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (member.roles.cache.has(ramal.vmutedRole)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** BU kullanıcı zaten seste susturulmuş!`)]}).sil(10)
        ...
        await member.roles.add(ramal.vmutedRole, `Ses Susturma, Yetkili: ${message.author.id}`)
        ...
        if (ramal.muteLog) client.channels.cache.get(ramal.muteLog).send({ embeds: [embed.setThumbnail(member.avatarURL({ dynamic: true })).setAuthor({ name : message.author.tag, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) }).setDescription(`${emojis.nokta} **Susturulan Kullanıcı :** ${member} (\`${member.user.username.replace(/\`/g, "")} - ${member.id}\`)\n${emojis.nokta} **Susturan Yetkili :** ${message.author} (\`${message.author.tag} - ${message.author.id}\`)\n${emojis.nokta} **Susturulma Zamanı :** \`${moment(Date.now()).format("LLL")}\`\n${emojis.nokta} **Susturulma Sebebi :** \`${reason}\`\n${emojis.nokta} **Susturulma Süresi :** \`${muteSure}\``).setTitle(`Ses Susturma İşlemi!`).setFooter(`Ceza ID : #${ceza.id}`)] })
