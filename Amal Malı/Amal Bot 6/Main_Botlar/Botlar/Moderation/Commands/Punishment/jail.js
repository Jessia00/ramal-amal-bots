...
class Jail extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (member.roles.cache.has(ramal.jailedRole)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** BU kullanıcı zaten cezalıya atılmış!`)]}).sil(10)
        ...
        await member.setRoles(ramal.jailedRole, `Cezalı, Yetkili: ${message.author.id}`)
        ...
        if (ramal.jailLog) client.channels.cache.get(ramal.jailLog).send({ embeds: [embed.setThumbnail(member.avatarURL({ dynamic: true })).setAuthor({ name : message.author.tag, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) }).setDescription(`${emojis.nokta} **Cezalıya Atılan Kullanıcı :** ${member} (\`${member.user.username.replace(/\`/g, "")} - ${member.id}\`)\n${emojis.nokta} **Cezalıya Atan Yetkili :** ${message.author} (\`${message.author.tag} - ${message.author.id}\`)\n${emojis.nokta} **Cezalıya Atılma Zamanı :** \`${moment(Date.now()).format("LLL")}\`\n${emojis.nokta} **Cezalıya Atılma Sebebi :** \`${reason}\``).setTitle(`Cezalandırma İşlemi!`).setFooter(`Ceza ID : #${ceza.id}`)] })
