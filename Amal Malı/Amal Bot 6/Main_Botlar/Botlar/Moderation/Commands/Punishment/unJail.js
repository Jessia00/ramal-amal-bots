...
class UnJail extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!member.roles.cache.has(ramal.jailedRole)) return message.reply({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten cezalıda değil!`)]}).sil(10)
        ...
        await member.setRoles(ramal.unregisterRole, `Cezalı Kaldırma, Yetkili: ${message.author.id}`)
