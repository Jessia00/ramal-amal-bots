...
\n **NOT: **Tagımızı alan kişilerin ismi 2 kere yazılıyor! \`${ramal.tag}\`
            `)]
            }).then(m => m.react("🎉")
            &&
            setTimeout(async () => {
                m.reactions.cache.get("🎉").users.cache.filter((u) => !u.bot && message.author.id !== u.id).map(x => {
                    if (x.username.includes(ramal.tag)) {
                      arr.push(x.id)
                    }
                    arr.push(x.id)
                  })
...
:tada: Emojisine basarak çekilişe katılabilirsin!
Çekiliş Süresi : ${saat == 0 ? `${dakika} dakika!` : `${saat} saat ${dakika} dakika!`}
\n **NOT: **Sadece tagı olanlar kazanabilir! \`${ramal.tag}\`
        `)]
        }).then(m => m.react("🎉")
        &&
        setTimeout(async () => {
            m.reactions.cache.get("🎉").users.cache.filter((u) => !u.bot && message.author.id !== u.id && u.username.includes(ramal.tag)).map(x => {
                arr.push(x.id)
              })
              shuffle(arr)
              let random = arr[Math.floor(Math.random() * arr.length)]
...
:tada: Emojisine basarak çekilişe katılabilirsin!
Çekiliş Süresi : ${saat == 0 ? `${dakika} dakika!` : `${saat} saat ${dakika} dakika!`}
\n **NOT: **Sadece seste bulunanlar kazanabilir! \`${ramal.tag}\`
        `)]
        }).then(m => m.react("🎉")
        &&
        setTimeout(async () => {
