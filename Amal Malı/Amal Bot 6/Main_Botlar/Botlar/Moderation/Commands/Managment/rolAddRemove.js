...
class RAddRemove extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (args[0].toLowerCase() === "ver") {
            ...
            client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${member} - ${member.id} kişisine ${message.author} - ${message.author.id} kişisi tarafından ${role} rolü verildi!`).setTimestamp()] })
            ...
            client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${member} - ${member.id} kişisinden ${message.author} - ${message.author.id} kişisi tarafından ${role} rolü alındı!`).setTimestamp()] })
        }
    }
}
