...
class Unregister extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (member.roles.cache.has(ramal.unregisterRole)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten kayıtsız!`)]}).sil(10)
        ...
        member.setRoles(ramal.unregisterRole)
        member.setNickname(`${ramal.isimsembol} Kayıtsız`)
        ...
        client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${member} (\`${member.user.tag} - ${member.id}\`) kişisi ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde kayıtsıza atıldı! ${emojis.onay}`)]})
