...
class Name extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!ramal.commandsChannel.some(kanal => message.channel.id.includes(kanal)) && message.channel.id !== ramal.welcomeChannel) return message.reply(`**UYARI:** Bu komutu yalnızca <#${ramal.welcomeChannel}> veya <#${ramal.commandsChannel[0]}> kanalında kullanabilirsin!`).sil(10)
        ...
        client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${member} \`(${member.id})\` adlı kullanıcının ismi ${message.author} \`(${message.author.id}\` tarafından ${new Date(message.createdAt).toTurkishFormatDate()} tarihinde \`${setName}\` olarak değiştirildi! ${emojis.onay}`)] })
