...
class GuildS extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (args[0] == 'kanaloluştur') {
            ...
            if (ramal.denetimLog) client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${message.member} [${message.member.id}] tarafından \`${args[1]}\` isminde kanal oluşturuldu! ${emojis.onay}`)] })
        }
        if (args[0] == 'isim') {
            ...
            if (ramal.denetimLog) client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${message.member} [${message.member.id}] tarafından sunucu ismi \`${args[1]}\` olarak değiştirildi! ${emojis.onay}`)] })
        }
        if (args[0] == 'resim') {
            ...
            if (ramal.denetimLog) client.channels.cache.get(ramal.denetimLog).send({ embeds: [embed.setDescription(`${message.member} [${message.member.id}] tarafından sunucu resmi değiştirildi! ${emojis.onay}`)] })
