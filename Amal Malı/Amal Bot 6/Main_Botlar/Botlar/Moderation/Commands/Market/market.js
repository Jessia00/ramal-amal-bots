const { Ramalcim, market, coins } = require('../../../../Helpers/Schemas')
class Market extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!args[0]) {
            if (!ramal?.urunler.length < 0) return message.channel.send({ content: `Şuan için market sisteminde herhangi bir ürün bulunmamakta! Eklendiğinde tekrar gelmeni umarım! ` }).sil(20)
            const row = new Discord.MessageActionRow().addComponents(
                new Discord.MessageSelectMenu().setCustomId('select').setPlaceholder('Market').addOptions(ramal?.urunler.map(x => [{ label: "Ürün : " + x.urunismi, description: x.urunfiyat + " Coin", value: `${x.urunismi}` }])).addOptions([{ label: "Kapat!", description: "Marketi Kapat!", value: "kapat" }])
            )
...
${emojis.star} Aşağıdan yalnızca \`Coin\`'inin yettiği ürünleri alabilirsin!
      `)]
            ...
            urunList.awaitMessageComponent({ filter: (component) => component.user.id === message.author.id, componentType: 'SELECT_MENU', }).then(async (interaction) => {
                if (ramal?.urunler.find((e) => e.urunismi == interaction.values[0])) {
                    let alıncakürün = ramal?.urunler.find((e) => e.urunismi == interaction.values[0])
                    if (alıncakürün.urunfiyat > Coin) {
                        ...
                        client.channels.cache.get(ramal.denetimLog).send(`${message.author} kişisi \`${interaction.values[0]}\` öğesini satın aldı! İletişime geçilmesini bekliyor!`)
                    }
                }
        ...
        if (args[0] == "ekle") {
            ...
            if (ramal?.urunler.length >= 24) return message.reply(`${emojis.iptal} Malesef daha fazla ürün ekleyemezsiniz!`).sil(50)
            ...
            if (!ramal.urunler) {
                await new Ramalcim({ guildID: message.guild.id, urunler: { urunismi: urunisim, urunfiyat: args[1] } }).save()
                message.react(emojis.onay)
                message.reply(`${emojis.onay} Başarılı bir şekilde \`${urunisim}\` adında \`${args[1]}\` fiyatında ürün eklendi!`).sil(50)
            } else {
                await Ramalcim.findOneAndUpdate({ guildID: message.guild.id }, { $push: { urunler: { urunismi: urunisim, urunfiyat: args[1] } } }, { upsert: true })
                message.react(emojis.onay)
                message.reply(`${emojis.onay} Başarılı bir şekilde \`${urunisim}\` adında \`${args[1]}\` fiyatında ürün eklendi!`).sil(50)
            }
        }
        if (args[0] == "sil") {
            ...
            if (!ramal?.urunler.map(x => x.urunismi == urunisim)) return message.reply(`${emojis.iptal} Böyle bir ürün bulamadım!`).sil(30)
            if (!ramal?.urunler) return message.reply(`${emojis.iptal} Belirli bir data bulunamadığı için bir ürün silemiyorum!`).sil(30)
            await Ramalcim.findOneAndUpdate({ $pull: { urunler: { urunismi: urunisim } } });
            message.react(emojis.onay)
