...
client.on("guildMemberOffline", async (member, oldStatus) => {
  ...
  const { preUser, Ramalcim } = require("../../Helpers/Schemas")
  ...
  if (member.manageable && !member.user.bot && member.guild.id === config.guildID && ["ADMINISTRATOR", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_CHANNELS", "MANAGE_WEBHOOKS"].some((a) => member.permissions.has(a))) {
    ...
    client.channels.cache.get(ramal.guardLog).send({
      embeds: [new Discord.MessageEmbed().setTitle("Yönetici Çevrimdışı Oldu!").setFooter(`Developed By Ramal.`).setTimestamp().setThumbnail(member.user.avatarURL())
        .setDescription(`\n
...
client.on("guildMemberOnline", async (member, newStatus) => {
  ...
  if (data.roles || data.roles.length) {
    ...
    client.channels.cache.get(ramal.guardLog).send({
      embeds: [new Discord.MessageEmbed().setTitle("Yönetici Çevrimiçi Oldu!").setFooter(`Developed By Ramal.`).setTimestamp().setThumbnail(member.user.avatarURL())
        .setDescription(`\n
