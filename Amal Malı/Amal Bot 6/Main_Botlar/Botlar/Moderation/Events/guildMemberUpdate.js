const { RoleData, Ramalcim } = require('../../../Helpers/Schemas')
                ...
                newMember.roles.cache.forEach(async role => {
                    if (!oldMember.roles.cache.has(role.id)) {
                        const emed = new Discord.MessageEmbed()
                            ...
                            .setTimestamp()
                        if(ramal.rolLog) client.channels.cache.get(ramal.rolLog).send({ embeds: [emed] })
                        RoleData.findOne({
                            user: hedef.id
                        }, async (err, res) => {
                            if (!res) {
                ...
                oldMember.roles.cache.forEach(async role => {
                    if (!newMember.roles.cache.has(role.id)) {
                        const emeed = new Discord.MessageEmbed()
                            ...
                            .setFooter(yapan.tag, yapan.displayAvatarURL({ dynamic: true }))
                            .setTimestamp()
                        if(ramal.rolLog) client.channels.cache.get(ramal.rolLog).send({ embeds: [emeed] })
                        RoleData.findOne({
                            user: hedef.id
                        }, async (err, res) => {
                            if (!res) {
