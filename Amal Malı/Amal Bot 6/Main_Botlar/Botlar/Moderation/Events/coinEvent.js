const { coins, Ramalcim } = require('../../../Helpers/Schemas');
...
class Ready {
    ...
    async run() {
        ...
        let kanal = client.channels.cache.get(ramal?.genelChat);
        setInterval(async () => {
            ...
            if (int == 2) {
                let fotolar = [
                    "https://cdn.discordapp.com/attachments/1007214128206712882/1007297728675586078/Ramalxramal.png"
                ]
                let kod = makeid(8)
                ...
                let kazan = await kanal.send({ content: `> [ Merhaba şanslı kişi! Aşağıdaki resimde yazan kodu ilk yazan olursan +1000 coin kazanacaksın! ]`, files: [attachment] })
                    .then(ramalmis => {
                        ...
                        let collector = ramalmis.channel.createMessageCollector({ filter: filter, time: 30000, max: 1, errors: ["time"] })
                        collector.on("collect", async (m) => {
                            let mesaj = m.content;
                            if (mesaj == kod) {
                                await coins.updateOne({ userID: uyecik.id }, { $inc: { "Coin": 1000 } }, { upsert: true }).exec();
                                if (ramalmis) ramalmis.delete();
