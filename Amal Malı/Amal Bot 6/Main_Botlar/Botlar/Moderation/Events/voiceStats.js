const { voiceJoinedAt, voiceUser, voiceGuild, voiceGuildChannel, voiceUserChannel, voiceUserParent, Ramalcim, coin } = require("../../../Helpers/Schemas")
class VoiceStats {
...
async function saveData(user, channel, data) {
    ...
    if (ramal.coinSystem === true && user.member.roles.cache.has(ramal.registerHammer)) {
        if (channel.parent && ramal.publicParents.includes(channel.parentID)) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: ((data / 1000 / 60) /config.voiceCount) *config.publicCoin } }, { upsert: true });
        ...
        if (coinData && coindatas.advancedRanks.some(x => x.coin >= coinData.coin)) {
            ...
            if (newRank && Array.isArray(newRank.role) && !newRank.role.some(x => user.member.roles.cache.has(x)) || newRank && !Array.isArray(newRank.role) && !user.member.roles.cache.has(newRank.role)) {
                const oldRank = coindatas.advancedRanks[ramal.advancedRanks.indexOf(newRank) - 1];
                ...
                user.guild.channels.cache.get(ramal.rankLog).send({ embeds: [embed.setDescription(`${user.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve ${Array.isArray(newRank.role) ? newRank.role.map(x => `<@&${x}>`).join(", ") : `<@&${newRank.role}>`} rolü verildi!`)] });
            }
        }
    }
    user.member.updateTask(user.guild.id, "ses", data, channel);
    if (ramal.dolarSystem === true) {
        if (channel.parent && ramal.publicParents.includes(channel.parentID)) {
