...
class MessageCreate {
  ...
  async run(message) {
    ...
    if (content === "tag" || content === ".tag" || content === "!tag") {
      ...
      message.channel.send(`\`Wea , wea , 神 , ? , #2019\``).sil(500)
    }
    ...
    if (!cmd) return;
    const embed = new Discord.MessageEmbed().setColor("RANDOM").setAuthor(message.author.username, message.author.avatarURL({ dynamic: true })).setTimestamp().setFooter(`Developed By Ramal.`, client.user.avatarURL({ dynamic: true }))
    if (config.root.includes(message.author.id)) try {
      cmd.run(client, message, args, embed).catch(error => { if (message) message.react(emojis.iptal); message.channel.send({ content: `Komut çalıştırılırken bir hata ile karşılaşıldı!\nHata: \`` + error + `\`\nHatanın çözümü için \`Ramal\` ile iletişime geçiniz!` }); })
    } catch (error) {
      ...
      if (ramal.commandLog) client.channels.cache.get(ramal.commandLog).send(`${emojis.cmd} ${message.author.username} (\`${message.author.id}\`) kişisi \`[${cmd.info.name}]\` isimli komutu çalıştırdı!`)
