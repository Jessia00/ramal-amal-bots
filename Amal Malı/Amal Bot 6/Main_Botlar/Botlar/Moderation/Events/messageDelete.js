...
const { Ramalcim, Snipes } = require("../../../Helpers/Schemas")
class MessageDelete {
  Event = "messageDelete"
  async run(message) {
    const ramal = await Ramalcim.findOne({ guildID: config.guildID })
    const channel = client.channels.cache.get(ramal.messageLog)
    if (!channel) return;
    ...
    .setFooter(`ID: ${message.author.id} | Developed by Ramal.`)
    .setTimestamp()
