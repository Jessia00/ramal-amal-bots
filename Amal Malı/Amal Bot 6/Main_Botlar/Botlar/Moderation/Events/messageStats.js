const { messageUser, messageGuild, messageGuildChannel, messageUserChannel, Ramalcim, coin } = require("../../../Helpers/Schemas")
const nums = new Map();
class MessageStats {
  Event = "messageCreate"
  async run(message) {
    try {
        ...
        if (ramal.dolarSystem === true) {
            if (message.channel.id !== ramal.genelChat) return;
            await coin.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { dolar: config.messageCoin } }, { upsert: true });
        }
        if (ramal.coinSystem === true && message.member.roles.cache.has(ramal.registerHammer)) {
            ...
            if (num && (num % config.messageCount) === 0) {
                ...
                if (coinData && coindatas.advancedRanks.some(x => coinData.coin === x.coin)) {
                    ...
                    message.guild.channels.cache.get(ramal.rankLog).send({ embeds: [embed.setDescription(`${message.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve ${Array.isArray(newRank.role) ? newRank.role.map(x => `<@&${x}>`).join(", ") : `<@&${newRank.role}>`} rolü verildi!`)] });
                }
            } else nums.set(message.author.id, num ? num + 1 : 1);
            if (message.channel.id == ramal.genelChat) {
