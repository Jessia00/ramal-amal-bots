...
config.Tokens.forEach(token => {
  ...
  async function urlSpam() {
    const ramal = await Ramalcim.findOne({ guildID: config.guildID }); const request = require('request'); const guild = clients.guilds.cache.get(config.guildID)
    if (!guild) return; if (guild.vanityURLCode && (guild.vanityURLCode == config.guildURL)) return;
    clients.channels.cache.get(ramal.guardLog).send({ content: `@everyone`, embeds: [new MessageEmbed().setDescription(`URL Değiştirildi. Kontrolüme Takıldı ve Bende Tekrardan URL'yi geri aldım.`)]})
...
clients.on("ready", async () => { const ramal = await Ramalcim.findOne({ guildID: config.guildID }); if(ramal.urlGuard === true) { setInterval(() => {  urlSpam(); }, 1000 * 5); } })
