...
module.exports = async (oldUser, newUser) => {
  if (oldUser.username !== newUser.username) {
    ...
    let totaltag = guild.members.cache.filter(member => member.user.username.includes(ayarlar.TAG) || member.user.username.includes(ayarlar.TAG3) || member.user.username.includes(ayarlar.TAG4) || member.user.username.includes(ayarlar.TAG5) || member.user.username.includes(ayarlar.TAG6) || member.user.username.includes(ayarlar.TAG7) || member.user.discriminator.includes(ayarlar.ETİKET) ).size;
    ...
    if (!log) return;
    if (newUser.username.includes(ayarlar.TAG) || newUser.username.includes(ayarlar.TAG3) || newUser.username.includes(ayarlar.TAG4) || newUser.username.includes(ayarlar.TAG5) || newUser.username.includes(ayarlar.TAG6) || newUser.username.includes(ayarlar.TAG7) || newUser.username.discriminator.toLowerCase().includes(ayarlar.ETİKET) && !user.roles.cache.has(ayarlar.TEAM)) {
...
${user} adlı üye tagımızı kullanıcı adına ekleyerek ailemize katıldı! | Sunucuda bulunan toplam taglı üyemiz: (${totaltag}) 
─────────────────
Önce: ${oldUser.tag} | Sonra: ${newUser.tag}`).catch();
    ...
    } else if (!newUser.username.includes(ayarlar.TAG3) && user.roles.cache.has(ayarlar.TEAM)) {
...
Bırakmadan önceki yetkileri:\n${user.roles.cache.filter(rol => ekipRol.position <= rol.position && !rol.managed).map(x => `<@&${x.id}>`)}
      ...
      if (ayarlar.TAGLOG && log) log.send(`
${user} adlı üye tagımızı kullanıcı adından silerek aramızdan ayrıldı! | Sunucuda bulunan toplam taglı üyemiz: (${totaltag}) 
─────────────────
Önce: ${oldUser.tag} | Sonra: ${newUser.tag}`).catch();
