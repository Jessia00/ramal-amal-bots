...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    let config = {
        "sevgilimvar": "969944836960567316",
        "sevgilimyok": "969945297021173780",
    }
    ...
    .setFooter(`ramalask ❤️ ${message.guild.name}`)
    .setColor("RED")
    .setDescription(`${member} adlı kullanıcının \n\n${client.emojis.cache.find(x => x.name === "ramal_stat")} İlişki Durumu: \`Sevgilisi Var\``)).then(msg => msg.delete({ timeout: 8000 }));
    
    if (member.roles.cache.has(config.sevgilimyok)) return message.channel.send(new MessageEmbed()
    .setFooter(`ramalask ❤️ ${message.guild.name}`)
    .setColor("PURPLE")
    .setDescription(`${member} adlı kullanıcının \n\n${client.emojis.cache.find(x => x.name === "ramal_stat")} İlişki Durumu: \`Sevgilisi Yok\``)).then(msg => msg.delete({ timeout: 8000 }));
    
    let embed = new MessageEmbed().setFooter(`ramalask ❤️ ${message.guild.name}`).setColor("BLACK").setDescription(`${member} adlı kullanıcı ilişki durumunu belirtmemiş \n\nAşağıdaki butonlardan ilişki durumunu belirte bilir! \n<#897215831786811473> Kanalındanda seçim yapabilirsin!`)
