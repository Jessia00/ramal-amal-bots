...
exports.run = async (client, message, args, durum, kanal) => {
  ...
  if (!client.ayarlar.sahip.some(x => x === message.author.id)) return
	if(args[0] === "kur" || args[0] === "setup" || args[0] === "kurulum") {
        guild.channels.create("ramalaz Log's", {type : "category"}).then(ct => {
        ...
        guild.channels.create("mute_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("vmute_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("jail_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("reklam_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("ban_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("warn_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("mission_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("ceza_puan_log", {type : "text"}).then(x => { x.setParent(ct.id) })
        guild.channels.create("ramal_veritabanı", {type : "text"}).then(x => { x.setParent(ct.id) })
