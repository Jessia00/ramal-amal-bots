...
let ses = client.convertDuration(data.totalVoice);
        let embed = new MessageEmbed().setColor("RANDOM").setFooter(`ramalask ❤️ ${message.guild.name}`)
...
.addField(`Ses Kanalları`,`${statemoji} **Sohbet Kanalları:** \`${client.convertDuration(pubOda)} (${parseInt(pubOda/(1000 * 60 * 60 * 1) * pubPuan)} puan)\`
...
.addField(`Puan Bilgileri`, `${statemoji} **Toplam Puan:** \`${(totalpoints+parseInt(ekPuan))}\`
${statemoji} **Ek Puan:** \`${ekPuan}\``)
.addField(`Ceza Kullanımı`, `\`\`\`diff
...
function progressBar(value, maxValue, size) {
    ...
    const progressText = `${client.emojis.cache.find(x => x.name == "ramal_ortabar")}`.repeat(progress);
    const emptyProgressText = `${client.emojis.cache.find(x => x.name == "ramal_griortabar")}`.repeat(emptyProgress);
    const bar = `${value ? client.emojis.cache.find(x => x.name == "ramal_solbar") : client.emojis.cache.find(x => x.name == "ramal_baslangicbar")}` + progressText + emptyProgressText + `${emptyProgress == 0 ? `${client.emojis.cache.find(x => x.name === "ramal_bitisbar")}` : `${client.emojis.cache.find(x => x.name === "ramal_gribitisbar")}`}`;
    return bar;
};
