...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.guild.owner.id === message.author.id || conf.sahip.some(member => message.author.id === member) || durum) {
        ...
        let embed = new MessageEmbed()
            ...
            .setTimestamp() // ramalaz ADAMDIR 
            .setDescription(`
\`\`\`Server Bilgileri\`\`\`
Sunucu Tag \`(${conf.prefix[0]}setup tag <serverTAG>)\`
Sunucu Tag2 Untag \`(${conf.prefix[0]}setup tag2 <serverTAG>)\`
Sunucu Tag3 \`(${conf.prefix[0]}setup tag3 <serverTAG>)\`
Sunucu Tag4 \`(${conf.prefix[0]}setup tag4 <serverTAG>)\`
Sunucu Tag5 \`(${conf.prefix[0]}setup tag5 <serverTAG>)\`
Sunucu Tag6 \`(${conf.prefix[0]}setup tag6 <serverTAG>)\`
Sunucu Tag7 \`(${conf.prefix[0]}setup tag7 <serverTAG>)\`
Sunucu EtiketTag \`(${conf.prefix[0]}setup etiket <serverTAG>)\`
...
`)
        ...
        await sunucuayar.findOne({guildID: conf.sunucuId}, async (err, data) => {
            ...
            if (["TAG3", "tag3", "Tag3"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.TAG3 = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
            if (["TAG4", "tag4", "Tag4"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.TAG4 = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
            if (["TAG5", "tag5", "Tag5"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.TAG5 = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
            if (["TAG6", "tag6", "Tag6"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.TAG6 = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
            if (["TAG7", "tag7", "Tag7"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.TAG7 = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
            if (["ETİKET", "etiket", "Etiket"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                data.ETİKET = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "ramal_tik"));
            };
...
Tag2 Untag ( **${y.TAG2 ? y.TAG2 : "\`Kapalı\`"}** )
Tag3 ( **${y.TAG3 ? y.TAG3 : "\`Kapalı\`"}** )
Tag4 ( **${y.TAG4 ? y.TAG4 : "\`Kapalı\`"}** )
Tag5 ( **${y.TAG3 ? y.TAG5 : "\`Kapalı\`"}** )
Tag6 ( **${y.TAG3 ? y.TAG6 : "\`Kapalı\`"}** )
Tag7 ( **${y.TAG3 ? y.TAG7 : "\`Kapalı\`"}** )
EtiketTag ( **${y.ETİKET ? y.ETİKET : "\`Kapalı\`"}** )
