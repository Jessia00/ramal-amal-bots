...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    let isim = args[1][0].toUpperCase() + args[1].substring(1);
        ...
        await message.guild.member(target.id).setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${isim}`).then(x => message.react(client.emojis.cache.find(x => x.name === "ramal_tik")))
        await teyit.updateOne({userID: target.id}, {$push: {userName: `\`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${isim}\` (İsim Güncellendi)`}}, {upsert:true}).exec()
    message.channel.send(new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${onay2} ${target} kullanıcısının ismi **${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${isim}** olarak değiştirildi!
