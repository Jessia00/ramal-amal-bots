...
message.channel.send(`Kısa sürede 5 den fazla kayıt yaptığınız için yetkileriniz çekildi.`)
        ...
        await target.roles.remove(unreg).then(async x => {
                await target.roles.add(erkekRol)
                await message.guild.member(target.id).setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name}`);
                ...
                await teyit.updateOne({
                    ...
                    $push: {
                        userName: `\`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name}\` (${erkekRol.map(x => `<@&${x}>`)})`
                    }
                }, {
                    upsert: true
                }).exec();
