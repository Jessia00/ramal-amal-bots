  ...
  if (isMemberFake) {
    ...
    Database.findOne({
      ...
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`:inbox_tray: ${member} sunucumuza katıldı! Davet Eden: **${inviter.id == member.guild.id ? member.guild.name : inviter.tag}** (\`${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}\`davet)`).catch(err => {});
        });
      } else {
